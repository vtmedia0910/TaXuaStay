...
export default async function AdminCmsPage({ params, searchParams }: { params: Promise<{ pageKey: string }>; searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  const [page, media, options] = await Promise.all([getAdminCmsPage(pageKey as CmsPageKey), getAdminCmsMedia(), getAdminCmsRoomOptions()]);
  return <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6"><AdminPageHeader title={pageKey === "home" ? "Nội dung Trang chủ" : "Nội dung Lưu trú"} description="Mọi thay đổi được lưu nháp trước; xem trước rồi xuất bản nguyên tử toàn trang." /><FormFeedback saved={feedback.saved} error={feedback.error} />{page ? <CmsPageEditor page={page} media={media} roomTypes={options.roomTypes} physicalRooms={options.physicalRooms} /> : <EmptyState title="Chưa có cấu trúc CMS" description="Hãy xác nhận migration 011 đã được áp dụng cho đúng Supabase project." />}</main>;
}
