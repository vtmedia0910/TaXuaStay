  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
      ...
      <Card className="mt-4 p-5 sm:p-6">
        ...
        {system.latestDraft ? <div className="mt-5 rounded-2xl border border-line p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-bold text-pine">Draft rev {system.latestDraft.revision} · {system.latestDraft.provider} · {system.latestDraft.model}</p><p className="mt-1 text-sm text-muted">Profile v{system.latestDraft.profile_revision} · Test {system.latestDraft.test_status}</p>{system.latestDraft.test_summary_code ? <p className="mt-1 break-all text-xs font-semibold text-muted">Safe diagnostic: {system.latestDraft.test_summary_code}</p> : null}</div>{activationReady ? <form action={activateAIRuntimeAction}><input type="hidden" name="runtime_revision" value={system.latestDraft.revision} /><input type="hidden" name="provider" value={system.latestDraft.provider} /><input type="hidden" name="model" value={system.latestDraft.model} /><SubmitButton label="Activate" icon={<Sparkles size={17} />} confirmation="Activate revision này cho customer runtime? Hard env gates vẫn tiếp tục áp dụng." /></form> : <button type="button" disabled className="min-h-11 rounded-full bg-mist px-4 text-sm font-bold text-muted">Activate bị khóa</button>}</div>{!activationReady ? <p className="mt-3 text-xs leading-5 text-muted">Cần credential Configured, health CONNECTED trong 24 giờ, Prompt Lab PASS, shared store + identity salt, environment cho phép và kill switch tắt.</p> : null}</div> : null}
      </Card>
