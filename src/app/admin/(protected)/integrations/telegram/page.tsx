...
import { telegramServerConfigStatus } from "@/features/telegram/bot";
...
export default async function TelegramIntegrationPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string; q?: string; status?: string }> }) {
  const [user, dashboard, query] = await Promise.all([requireAdminUser(), getTelegramDashboard(), searchParams]);
  ...
  return <main className="mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
    ...
    <TelegramDashboard dashboard={{ ...dashboard, suppliers: filtered }} role={user.role} configured={telegramServerConfigStatus()} />
  </main>;
}
