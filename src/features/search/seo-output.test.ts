  ...
  it("builds sitemap entries from public records without admin routes", () => {
    const sitemap = buildPublicSitemap(
      ...
      ["/", "/stay", "/homestay-ta-xua"],
      {
        ...
        motorbikes: [{ slug: "public-bike", updated_at: "2026-08-29T00:00:00Z" }],
        packages: [{ slug: "public-package", updated_at: "2026-08-29T00:00:00Z" }],
      },
    );
    ...
    expect(sitemap.map((entry) => entry.url)).toContain("https://stay.example/motorbike/public-bike");
    expect(sitemap.map((entry) => entry.url)).toContain("https://stay.example/packages/public-package");
