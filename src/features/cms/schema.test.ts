...
import { cmsItemSchema, cmsMediaQuerySchema, cmsPageSchema, cmsSectionSchema, externalCmsMediaSchema, updateCmsMediaSchema } from "@/features/cms/schema";
...
describe("structured CMS validation", () => {
  ...
  it("rejects unsafe CTA schemes and ignores client-supplied section order", () => {
    ...
    const parsed = cmsSectionSchema.safeParse({ ...base, cta_href: "/stay" });
    expect(parsed.success).toBe(true);
    if (parsed.success) expect(parsed.data).not.toHaveProperty("sort_order");
  ...
  it("never lets a hybrid block copy two room entity scopes", () => {
    ...
    const parsed = cmsItemSchema.safeParse({ ...base, room_type_id: entity, physical_room_id: "" });
    expect(parsed.success).toBe(true);
    if (parsed.success) expect(parsed.data).not.toHaveProperty("sort_order");
  ...
  it("requires useful alt text and HTTPS external media", () => {
    const base = { title: "Hero săn mây", alt_text: "Dãy núi Tà Xùa trong nắng sớm", caption: "", role: "hero", focal_x: "50", focal_y: "42" };
  ...
  it("bounds media-library queries and focal-point updates", () => {
    expect(cmsMediaQuerySchema.safeParse({ query: "mây", role: "hero", page: "2", pageSize: "24" }).success).toBe(true);
    expect(cmsMediaQuerySchema.safeParse({ query: "x".repeat(81), role: "all", page: 1, pageSize: 24 }).success).toBe(false);
    const metadata = { id: "00000000-0000-4000-8000-000000000001", title: "Hero", alt_text: "Mây trên núi", caption: "", role: "hero", focal_x: 101, focal_y: 50 };
    expect(updateCmsMediaSchema.safeParse(metadata).success).toBe(false);
  });
});
