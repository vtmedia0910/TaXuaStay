...
import {
  cmsIdSchema, cmsItemSchema, cmsPageSchema, cmsSectionSchema,
  externalCmsMediaSchema, uploadCmsMediaSchema,
...
export async function saveCmsItemAction(formData: FormData) {
  await requireAdminUser();
  const returnPath = contentPath(formData.get("page_key"));
  const parsed = cmsItemSchema.safeParse(Object.fromEntries(formData));
  if (!parsed.success) redirect(`${returnPath}?error=cms-item-invalid`);
  const client = await createServerSupabaseClient();
  if (!client) redirect(`${returnPath}?error=config`);
  const { id, page_key, ...draft } = parsed.data;
  const result = id
    ? await client.from("cms_section_items").update(draft).eq("id", id)
    : await client.from("cms_section_items").insert(draft);
...
export async function archiveCmsPageAction(formData: FormData) {
  ...
  const client = await createServerSupabaseClient();
  if (!client) redirect(`/admin/content/${parsed.data.page_key}?error=config`);
  const { error } = await client.from("cms_pages").update({ status: "archived" }).eq("page_key", parsed.data.page_key);
...
function mediaFields(formData: FormData) {
  return {
    ...
    role: formData.get("role"), width: formData.get("width"), height: formData.get("height"),
    focal_x: formData.get("focal_x"), focal_y: formData.get("focal_y"),
  };
}
