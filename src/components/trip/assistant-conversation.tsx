        ...
        {messages.length === 0 ? (
          <div className={`border border-line bg-white shadow-sm ${embedded ? "rounded-3xl p-4" : "rounded-[1.75rem] p-5"}`}>
            <div className="flex items-center gap-2 font-bold text-pine">
              ...
              {hardUnavailable ? "Trợ lý chưa sẵn sàng" : "Bạn muốn biết điều gì?"}
            </div>
            <p className="mt-2 text-sm leading-6 text-muted">
              {unavailableCopy ?? "Mình có thể hỗ trợ chọn phòng, đọc dữ kiện đã xác minh, giá, tình trạng phòng, gói dịch vụ, chính sách công khai và My Trip khi phiên này đã được cấp quyền."}
            </p>
            {!hardUnavailable ? (
