...
export function TripHeader() {
  return (
    <header className="sticky inset-x-0 top-0 z-50 border-b border-line bg-white/95 backdrop-blur-md">
      <div className="mx-auto flex min-h-18 max-w-7xl items-center gap-3 px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-0.5 xl:flex" aria-label="Điều hướng chính">
          {navigation.map((item) => (
            <Link key={item.label} href={item.href} className="inline-flex min-h-11 items-center rounded-full px-3 text-sm font-semibold text-ink hover:bg-pine-soft hover:text-pine">
              {item.label}
            </Link>
          ))}
        </nav>

        <Link href={PUBLIC_ROUTES.stay} className={buttonVariants({ size: "sm", className: "max-sm:hidden" })}>
          <Search size={17} aria-hidden="true" />Tìm chuyến đi
        </Link>

        <details className="trip-mobile-menu group relative xl:hidden">
          <summary className="grid size-11 cursor-pointer list-none place-items-center rounded-full border border-line text-pine marker:hidden" aria-label="Mở menu">
