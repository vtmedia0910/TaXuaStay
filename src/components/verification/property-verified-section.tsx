...
export function PropertyVerifiedSection({ bundle }: { bundle: PropertyVerificationBundle }) {
  ...
  return (
    <section aria-labelledby="property-verified-title">
      <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-bold uppercase tracking-[0.14em] text-copper-strong">Tà Xùa Trip · đã thẩm định</p><h2 id="property-verified-title" className="mt-2 font-display text-3xl font-bold text-pine">Thông tin đã được kiểm tra</h2></div>{propertyVerified ? <Badge className="bg-pine text-white"><ShieldCheck size={15} />Nơi lưu trú đã thẩm định</Badge> : null}</div>
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        {propertyVerified ? <Card className="p-5"><CheckCircle2 className="text-success" aria-hidden="true" /><h3 className="mt-3 font-bold text-pine">Danh tính & vị trí đã thẩm định</h3><p className="mt-2 text-sm leading-6 text-muted">Danh tính được kiểm tra ngày {formatDate(propertyVerified.identity.verified_at)}; vị trí được kiểm tra ngày {formatDate(propertyVerified.location.verified_at)}. Kiểm tra lại trước mốc sớm hơn: {formatDate(propertyVerified.identity.expires_at < propertyVerified.location.expires_at ? propertyVerified.identity.expires_at : propertyVerified.location.expires_at)}.</p></Card> : null}
        {bundle.cloudVerifiedRoomCount > 0 ? <Card className="p-5"><Mountain className="text-copper" aria-hidden="true" /><h3 className="mt-3 font-bold text-pine">{bundle.cloudVerifiedRoomCount} loại phòng có Cloud View đã thẩm định</h3><p className="mt-2 text-sm leading-6 text-muted">Số lượng được tính từ các hồ sơ xác minh phòng còn hiệu lực, không nhập thủ công.</p></Card> : null}
      </div>
      {road ? (
        <Card className="mt-4 p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-bold uppercase tracking-[0.12em] text-copper-strong">Đường vào đã thẩm định</p><h3 className="mt-1 font-display text-2xl font-bold text-pine">{ROAD_GRADE_LABELS[road.grade]}</h3></div><Badge className="text-success">Xác minh {formatDate(road.verified_at)}</Badge></div>
