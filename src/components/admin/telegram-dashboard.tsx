import { AlertTriangle, Bot, CheckCircle2, Clock3, Send, UsersRound } from "lucide-react";
...
import type { TelegramDashboard as Dashboard } from "@/features/telegram/types";

function dateTime(value: string | null) {
  return value ? new Intl.DateTimeFormat("vi-VN", { dateStyle: "short", timeStyle: "short" }).format(new Date(value)) : "Chưa có";
}
export function TelegramDashboard({ dashboard, role, configured }: { dashboard: Dashboard; role: AdminRole; configured: { botTokenConfigured: boolean; webhookSecretConfigured: boolean } }) {
  ...
  return <div className="grid gap-6">
    ...
    {!configured.botTokenConfigured || !configured.webhookSecretConfigured ? <Card className="border-amber-200 bg-amber-50 p-4"><p className="flex items-center gap-2 font-bold text-amber-950"><AlertTriangle size={18} />Chưa đủ cấu hình server</p><p className="mt-2 text-sm text-amber-950">Cần cả TELEGRAM_BOT_TOKEN và TELEGRAM_WEBHOOK_SECRET trong Vercel. Giá trị secret không được hiển thị ở đây.</p></Card> : null}
