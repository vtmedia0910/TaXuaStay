...
Private suppliers, normalized contacts, Property relationships, Partner lifecycle/tier, and opaque external identities are implemented as the V2 Phase 3 supply-side foundation. Phase 3H migration 017 hardens atomic archive ordering and ID-preserving primary-contact edits. Phase 4 migrations 018–020 add private accommodation net cost, market reference and internal contribution/margin diagnostics while preserving the existing public sell-price engine and denying anonymous helper execution. Phase 5 migrations 021–022 add a bounded manual/reference motorbike catalog, its public ordering contract, and `/motorbike`; they do not copy fleet operations or claim live availability. Phase 6 migration 023 adds generic Package composition, explicit Package sell pricing and private Package economics. Application-only Phase 7 adds deterministic, explainable `/trip-finder`. Phase 8 migrations 024–026 add one private Booking per traveler trip, many immutable Booking Items, separate Supplier Confirmation, append-only events, `/booking/request`, protected status and `/admin/bookings`. Payments, deposit, checkout, bus integration and the Trip Dashboard remain future architecture only. Tà Xùa Biker remains independent and the source of truth for motorbike operations.
...
- `docs/V2_PHASE_6_PACKAGE_COMMERCE.md` — generic Package composition, explicit sell-price authority, availability/confirmation aggregation, private economics, Admin/public routes and security boundary.
- `docs/V2_PHASE_7_TRIP_FINDER.md` — intent inputs, deterministic recommendation policy, candidate composition, explanations, mobile UX, SEO, security and no-booking boundary.
- `docs/V2_PHASE_8_UNIFIED_BOOKING_SUPPLIER_CONFIRMATION.md` — atomic Booking submission, immutable items/snapshots, Supplier Confirmation, RLS, public status and Admin operations.
