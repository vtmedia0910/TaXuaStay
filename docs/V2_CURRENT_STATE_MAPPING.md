...
This document bridges the **Legacy Foundation Completed** (migrations 001–008) and completed V2 Phases through Phase 9 to the target architecture in Master Plan V2.1. “Target” means planned future capability, not current application behavior.
...
| Versioned quotes/deposit policies and provider-unconfigured checkout sessions | Item-aware payment/deposit, transport services, Trip Operations, and Trip Dashboard | Checkout Readiness implemented in V2 Phase 9 without collecting money. Real payment provider/state, bus, Trip Operations and Trip Dashboard remain future. Payment remains distinct from Booking/Supplier Confirmation/readiness. |
...
Generic standalone Services, motorbike fleet operations/live integration, Bus, a real payment provider, QR/link, webhook, payment transaction/paid state, refund, payout, settlement, full My Trip, Trip Operations, and Trip Dashboard do not exist. Trip Finder itself creates no request record, hold, Booking, profile or payment; only an explicit Phase 8 request creates a Booking. Phase 9 readiness/checkout drafts never collect money. Transport remains clearly marked future. Combo is shown as usable only when at least one genuine published Package passes public eligibility; otherwise it remains `Sắp có`. Supplier/Partner status and private economics do not change verification, price confidence, availability, search ranking, Trip Finder ranking or checkout readiness.
