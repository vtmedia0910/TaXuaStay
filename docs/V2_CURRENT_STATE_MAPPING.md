...
This document bridges the **Legacy Foundation Completed** (migrations 001–008) and completed V2 Phases through Phase 11 to the target architecture in Master Plan V2.1. “Target” means planned future capability, not current application behavior.
...
Generic standalone Services, motorbike fleet operations/live integration, Bus, a real payment provider, QR/link, payment webhook, payment transaction/paid state, refund, payout, settlement, customer accounts and AI Customer Assistant do not exist. Trip Finder itself creates no request record, hold, Booking, profile or payment; only an explicit Phase 8 request creates a Booking. Phase 9 readiness/checkout drafts never collect money, Phase 10 only projects existing truth, and Phase 11 only operates those states. Phase 12 automates Supplier communication through one shared Telegram bot and private mapped groups while leaving database Confirmation/Operations authoritative. Transport remains clearly marked future. Combo is shown as usable only when at least one genuine published Package passes public eligibility; otherwise it remains `Sắp có`. Supplier/Partner status and private economics do not change verification, price confidence, availability, search ranking, Trip Finder ranking, checkout readiness or operational priority.
