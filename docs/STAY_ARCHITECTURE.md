...
Application-only Phase 13E adds a conservative social-intent fast path before runtime/admission, a strict `phase13e-v1` browser-session advisor state, deterministic preference extraction, consultation stages, one next-best question, bounded public option references and contextual follow-up resolution. Pure social turns use no provider, tool, token or paid Upstash admission. Grounded questions continue through the existing runtime and exactly nine read-only tools. Behavior Studio offers an advisor template that only fills a DRAFT form and never activates it. No long-term profile, new transcript behavior, write authority, provider or migration 035 is added. See `docs/V2_PHASE_13E_CONVERSATIONAL_ADVISOR_SMART_CONSULTATION.md`.
