...
No runtime call site requires a service-role client. The tracked environment template contains the final canonical URL, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, plus Phase 12's server-only `TELEGRAM_BOT_TOKEN` and `TELEGRAM_WEBHOOK_SECRET`. Neither `SUPABASE_SERVICE_ROLE_KEY` nor `SUPABASE_SECRET_KEY` belongs in the application or Vercel environment.
