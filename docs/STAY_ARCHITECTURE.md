...
No runtime call site requires a service-role client. The tracked environment template contains the final canonical URL, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, Phase 12's server-only `TELEGRAM_BOT_TOKEN` and `TELEGRAM_WEBHOOK_SECRET`, plus the provider-neutral server-only Phase 13 placeholders `AI_PROVIDER`, `AI_MODEL`, and `AI_API_KEY`. Neither `SUPABASE_SERVICE_ROLE_KEY` nor `SUPABASE_SECRET_KEY` belongs in the application or Vercel environment. AI credentials never enter browser code, database rows, logs or Admin output.
...
Application-only Phase 13 adds `/assistant`, a non-indexable mobile-first text experience, a floating public launcher, `/api/assistant`, and Admin-only `/admin/integrations/ai`. The provider boundary is typed and configuration-driven. No provider was owner-configured at implementation time, so Production fails closed with a useful fallback instead of selecting a vendor or sending data externally.
...
There is no generic database/RPC tool, direct Supabase credential, write tool, service-role, provider Secret key, private corpus, embedding store, persistent conversation memory or migration 034. Rate limiting, bounded history/input/output/tool calls, request/provider timeouts, prompt-injection refusal, DTO sanitization and fail-closed errors form the application boundary. See `docs/V2_PHASE_13_AI_CUSTOMER_ASSISTANT.md`.
