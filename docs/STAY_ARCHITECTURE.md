...
Application-only Phase 13E evaluates the code-owned master environment gate before its conservative social/action fast paths. Once allowed, those paths run before runtime resolution/admission; pure social turns use no provider, tool, token or paid Upstash admission. A strict `phase13e-v1` browser-session advisor state adds deterministic preference extraction, consultation stages, one next-best question, bounded public option references and contextual follow-up resolution. Grounded questions continue through the existing runtime and exactly nine read-only tools. Behavior Studio offers an advisor template that only fills a DRAFT form and never activates it. No long-term profile, new transcript behavior, write authority, provider or migration 035 is added. See `docs/V2_PHASE_13E_CONVERSATIONAL_ADVISOR_SMART_CONSULTATION.md`.
