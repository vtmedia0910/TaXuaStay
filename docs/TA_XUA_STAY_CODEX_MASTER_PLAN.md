...
Phase 13 is complete without a database migration. `/assistant` and `/api/assistant` provide a bounded, mobile-first, read-only customer assistant over nine validated, customer-safe tools. Deterministic Trip Finder, current public price/availability/verification projections, Package price authority and the existing code-plus-opaque-token My Trip projection remain authoritative. Phase 13A supplies shared atomic Upstash rate/budget admission, usage/cost accounting and kill switch. Phase 13B uses metadata-only migration 034 for a code-owned Gemini/OpenAI/DeepSeek registry, immutable Draft/Test/Activate/Rollback runtime revisions, explicit health, Behavior Studio and Prompt Lab. It seeds no ACTIVE runtime and does not activate paid inference. Production stays disabled until owner configuration and green health/Prompt Lab/smoke verification. The model has no direct SQL/Supabase/private-table access and no write tool. See `docs/V2_PHASE_13_AI_CUSTOMER_ASSISTANT.md`, `docs/V2_PHASE_13A_AI_PROVIDER_ACTIVATION_COST_HARDENING.md` and `docs/V2_PHASE_13B_MULTI_PROVIDER_AI_BEHAVIOR_STUDIO.md`.
...
Phase 13E is complete without a database migration. It adds a conservative pure-social fast path before runtime/admission, a strict session-only advisor state, deterministic intent/preference extraction, staged consultation, one next-best question, contextual option references and concise suggested replies. Mixed/domain prompts continue through the locked runtime and exactly nine read-only tools; no long-term customer profile or write authority is added. See `docs/V2_PHASE_13E_CONVERSATIONAL_ADVISOR_SMART_CONSULTATION.md`.
