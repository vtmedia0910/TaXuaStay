...
**Current product state:** Legacy Foundation 001–008 + V2 Phase 1 / migration 009 + V2 Phase 2 / migration 010 + application-only V2 Phase 2.5 + V2 Phase 2.6 / migrations 011–014 + V2 Phase 2.6H / migration 015 + V2 Phase 3 / migration 016 + corrective V2 Phase 3H / migration 017 + V2 Phase 4 / migrations 018–020 + V2 Phase 5 / migrations 021–022 + V2 Phase 6 / migration 023 + application-only V2 Phase 7 + V2 Phase 8 / migrations 024–026 + V2 Phase 9 / migrations 027–028 + V2 Phase 10 / corrective migration 029 + V2 Phase 11 / migrations 030–032 + V2 Phase 12 / migration 033 + application-only V2 Phase 12H + application-only V2 Phase 13 + Phase 13A controls + V2 Phase 13B / migration 034 + application-only V2 Phase 13C, 13D and 13E đã hoàn thành ở codebase. Phase 13B adds a code-owned Gemini/OpenAI/DeepSeek registry, immutable runtime/profile revisions, Behavior Studio and Prompt Lab while retaining the nine read-only tools and Phase 13A shared quota/budget/kill-switch controls. Phase 13D adds a dedicated redacted transcript store whose logging defaults OFF and whose failure cannot block inference. Phase 13E adds a deterministic zero-provider social path and strict session-only advisor state, stages, one next-best question and bounded public option references without new tools or persistent customer profiling. Migration 034 contains metadata only and creates no ACTIVE runtime; no migration 035 exists. Production AI state is controlled by `AI_ENABLED`, `AI_KILL_SWITCH`, the ACTIVE runtime, provider health and the existing Phase 13A/13B gates; operators must check live Vercel and Admin state before operational changes. The model has no direct Supabase/private-table access or write tool. Chưa có Payment transaction/paid state/QR/link/payment webhook/refund/settlement, customer account hoặc service vertical mới. Phase 12Q Staging/E2E QA infrastructure vẫn deferred theo quyết định owner.

**Latest application extension:** V2 Phase 13E — Conversational Advisor & Smart Consultation adds deterministic social routing, strict bounded browser-session preferences, consultation stages, one next-best question and contextual public-option pointers above the locked Phase 13–13D runtime. Pure social turns bypass provider and paid admission; grounded turns still use exactly nine read-only tools. Migrations remain 001–034.
...
Phase 10 is complete with narrow corrective migration 029. The existing code-plus-opaque-token `/booking/[bookingCode]` surface is the canonical My Trip experience and projects immutable Booking Item presentation facts, four customer-safe verification fields derived from the immutable snapshot, Supplier Confirmation, current quote/deposit/readiness, customer-safe events and real configured support paths through `CustomerTripDashboardDto`. The RPC returns no raw verification snapshot, PII or private economics; anonymous table grants and RLS remain unchanged. It adds no customer account, duplicate lifecycle or payment behavior. Customer routes remain noindex and outside sitemap. See `docs/V2_PHASE_10_MY_TRIP_CUSTOMER_DASHBOARD.md`.

Phase 13 is complete without a database migration. `/assistant` and `/api/assistant` provide a bounded, mobile-first, read-only customer assistant over nine validated, customer-safe tools. Deterministic Trip Finder, current public price/availability/verification projections, Package price authority and the existing code-plus-opaque-token My Trip projection remain authoritative. Phase 13A supplies shared atomic Upstash rate/budget admission, usage/cost accounting and kill switch. Phase 13B uses metadata-only migration 034 for a code-owned Gemini/OpenAI/DeepSeek registry, immutable Draft/Test/Activate/Rollback runtime revisions, explicit health, Behavior Studio and Prompt Lab. Migration 034 itself seeds no ACTIVE runtime and does not activate paid inference. Current Production AI behavior is governed operationally by `AI_ENABLED`, `AI_KILL_SWITCH`, the ACTIVE runtime, provider health and all existing Phase 13A/13B gates; live Vercel/Admin state must be checked before changes. The model has no direct SQL/Supabase/private-table access and no write tool. See `docs/V2_PHASE_13_AI_CUSTOMER_ASSISTANT.md`, `docs/V2_PHASE_13A_AI_PROVIDER_ACTIVATION_COST_HARDENING.md` and `docs/V2_PHASE_13B_MULTI_PROVIDER_AI_BEHAVIOR_STUDIO.md`.

Phase 13C is complete without a database migration. It adds a persistent launcher, a versioned browser-local welcome teaser with a five-day cooldown, and a lazy mobile bottom sheet/desktop panel that reuse the existing Assistant conversation and API. Page load, teaser, launcher and panel open make no provider call; only explicit prompt submission may enter the locked runtime. Public page context is a narrow routing hint, sensitive routes are suppressed, and disabled readiness renders truthful public fallbacks. See `docs/V2_PHASE_13C_ASSISTANT_DISCOVERY_EMBEDDED_CHAT_UX.md`.

Phase 13D is complete without a Supabase migration. It adds best-effort redacted transcript observability in a separate dedicated Redis store, bounded TTL/deletion and Admin-only review while leaving customer inference independent of logging failure. See `docs/V2_PHASE_13D_AI_CONVERSATION_OBSERVABILITY_RETENTION.md`.

Phase 13E is complete without a database migration. It checks the master environment gate before its conservative pure-social/action fast paths; once allowed, those deterministic paths still run before runtime resolution and paid admission. It adds a strict session-only advisor state, deterministic intent/preference extraction, staged consultation, one next-best question, contextual option references and concise suggested replies. Mixed/domain prompts continue through the locked runtime and exactly nine read-only tools; no long-term customer profile or write authority is added. See `docs/V2_PHASE_13E_CONVERSATIONAL_ADVISOR_SMART_CONSULTATION.md`.
