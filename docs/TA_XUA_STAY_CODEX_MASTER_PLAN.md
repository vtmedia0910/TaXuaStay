...
Phase 13 is complete without a database migration. `/assistant` and `/api/assistant` provide a bounded, mobile-first, read-only customer assistant over nine validated, customer-safe tools. Deterministic Trip Finder, current public price/availability/verification projections, Package price authority and the existing code-plus-opaque-token My Trip projection remain authoritative. Phase 13A selects exactly the OpenAI Responses adapter and `gpt-5-mini-2025-08-07`, with shared atomic Upstash rate/budget admission, usage/cost accounting, kill switch and Admin-only explicit health diagnostics. Production paid inference stays disabled until owner configuration and green health/smoke verification. The model has no direct SQL/Supabase/private-table access and no write tool. See `docs/V2_PHASE_13_AI_CUSTOMER_ASSISTANT.md` and `docs/V2_PHASE_13A_AI_PROVIDER_ACTIVATION_COST_HARDENING.md`.
