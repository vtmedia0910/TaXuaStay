...
Phase 13 is complete without a database migration. `/assistant` and `/api/assistant` provide a bounded, mobile-first, read-only customer assistant over nine validated, customer-safe tools. Deterministic Trip Finder, current public price/availability/verification projections, Package price authority and the existing code-plus-opaque-token My Trip projection remain authoritative. The model has no direct SQL/Supabase/private-table access and no write tool. Provider selection remains owner-controlled; because no provider was configured during implementation, the deployed assistant degrades to a safe unavailable state with existing product links. See `docs/V2_PHASE_13_AI_CUSTOMER_ASSISTANT.md`.
